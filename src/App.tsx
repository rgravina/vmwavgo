...
function App() {
  ...
  return (
    <div className="App">
      ...
      <main>
        <div className="form">
        <div>You had: <input data-testid="vmw" value={vmwStock} maxLength={5} onChange={(e) => setVmwStock(Number(e.target.value))}/> <span className="vmw">VMW</span>.</div>
        <div>You have: <input data-testid="avgo" value={avgoStock} maxLength={5} onChange={(e) => setAvgoStock(Number(e.target.value))}/> <span className="avgo">AVGO</span>.</div>
        </div>
        <div>
        <div className="stock">
          <h2>Stock Portion</h2>
          <ul>
            <li data-testid="stock">{avgoPercent.toFixed(2)}% of your <span className="vmw">VMW</span> is <span className="avgo">AVGO</span> now (at a conversion rate of {avgo.stockConversionRate} <span className="avgo">AVGO</span> per <span className="vmw">VMW</span> share).</li>
          ...
          <div className="cash">
            <h2>Cash Portion</h2>
            <ul>
              <li data-testid="cash">{cashPercent.toFixed(2)}% of your <span className="vmw">VMW</span> was converted to cash at a price of {usdFormatter.format(vmw.priceAquired)}.</li>
          ...
          <h2>Other Info</h2>
          <ul>
            <li data-testid="stock-peak">Your <span className="vmw">VMW</span> was worth {usdFormatter.format(vmwStock * vmw.priceBeforeClose)} before conversion when it hit its peak of {usdFormatter.format(vmw.priceBeforeClose)}.</li>
            <li data-testid="all-cash">Your <span className="vmw">VMW</span> would be worth {usdFormatter.format(vmwStock * vmw.priceAquired)} if you got all cash at a price of {usdFormatter.format(vmw.priceAquired)}.</li>
          </ul>
        </div>
      </main>
    </div>
